...
public class GridDisplay extends JPanel {
	...
	private void paintSpot(Graphics aGraphics, int row, int col, float t) {
		...
		// TODO: since we know we're going to update all spots every time, it
		// may be more
	...
	protected void paintComponent(Graphics aGraphics) {
		...
		if (null != curGrid) {
			...
			// calculate scale factor in case we need to sub-sample the grid
			// output
			...
			for (int i = 0; i < panelH; i++) {
				for (int j = 0; j < panelW; j++) {
					paintSpot(anotherGraphics, i, j, curGrid.getTemperature(
							(int) Math.floor(i * xScale),
							(int) Math.floor(j * yScale)));
				}
			}
		}
